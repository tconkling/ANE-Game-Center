...
void AirGameCenter_FPANE_Log(FREContext context, NSString* message) {
    ...
    AirGameCenter_FPANE_DispatchEventWithInfo(context, @"LOGGING", message);
}
...
NSArray* AirGameCenter_FPANE_FREObjectToNSArrayOfUIImage(FREObject object) {
    ...
    for (NSInteger i = 0; i < arrayLength; i++) {
        ...
        UIImage *image = AirGameCenter_FPANE_FREBitmapDataToUIImage(itemRaw);
...
FREObject AirGameCenter_FPANE_CreateError(NSString* error, NSInteger* id) {
    ...
    FREObject argV[] = {
            AirGameCenter_FPANE_NSStringToFREObject(error),
            freId
    };
